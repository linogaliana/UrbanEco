...
One of the standard ways to combine dataframes is by `inner join`. For two dataframes, the `dplyr::left_join` function allows straightforward merge using unique `ids`/`keys`. With a very close syntax, this methodology can be applied to create inner joins between `Spatial` objects and standard dataframes by associating each observation in a table with a geometric object (a polygon, line, or point). In particular, `merge` or `left_join` functions can be used by adapting standard syntax to `Spatial` objects. 
